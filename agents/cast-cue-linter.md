...
*(Illustrative §5 patterns: a single character cued under multiple personas or life-stages by design (e.g. one body whose cue changes by persona, or a YOUNG \<NAME\>/\<NAME\> pairing for the same role at different ages); a code-name that resolves to a real name partway through (e.g. CODENAME=REALNAME after a reveal); a full-name/short-name pair (e.g. \<FIRST LAST\>/\<FIRST\>) plus a declared known typo to ignore. A different screenplay has its own systems — or none, in which case name inconsistencies like MIKE↔MARK or two names for one character are real findings, not aliases.)*

## What to check

1. **Cue spelling & casing consistency (core job).** Group every dialogue cue by character; flag the same character cued under differing spellings, casings, or trailing punctuation (e.g. a cue appearing as `NAME`, `FULL NAME`, and `NAME.`). Present the canonical cue, each variant, and every scene # where each appears.
